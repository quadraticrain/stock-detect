...
> **重要原则（v5.7 语义优先）**：本任务是 **AI 语义分析任务**，不要把语义判断下沉成 Python/SQL 关键词规则或机械化脚本。脚本只负责 MySQL 读写、断点、幂等与 JSON 组装；`recommendation`、`confidence`、`reasoning`、Ticker 是否应映射，必须尽量由 AI 直接阅读 `stock_detect_x_posts.text` 原文后判断。代码可以用于分页、格式化、去重、候选检索和写表，但不能替代 AI 对近义词、反讽、上下文、行业语境、公司/产品歧义的理解。
...
FROM stock_detect_ai_runs
WHERE account IN ('aleabitoreddit', 'mingchikuo', 'xueqiu:1247347556', 'xueqiu:1102105103') AND checkpoint_post_id IS NOT NULL
ORDER BY analyzed_at DESC LIMIT 1;

-- 确认无重复分析
SELECT post_id, COUNT(DISTINCT run_id) AS runs
FROM stock_detect_ai_signals
WHERE account IN ('aleabitoreddit', 'mingchikuo', 'xueqiu:1247347556', 'xueqiu:1102105103')
GROUP BY post_id
HAVING runs > 1
LIMIT 20;
```
